...
const CustomNode = ({ data, selected }: NodeProps) => {
  return (
    <div className={`relative flex items-center gap-3 w-64 p-3 rounded-xl bg-white/95 backdrop-blur-md transition-all duration-300 overflow-visible ${selected ? 'shadow-lg ring-red-400 ring-2 ring-offset-2 z-10 scale-[1.02] border-transparent' : 'border border-red-200 shadow-sm hover:shadow-md hover:ring-red-400 hover:ring-1 hover:border-transparent'}`}>
      ...
      <div className="flex flex-col min-w-0 flex-1 pl-2">
        ...
        <div className="text-[10px] font-mono text-slate-500 truncate" title={data.location as string}>
...
const PathHeaderNode = ({ data }: NodeProps) => {
  return (
    <div className="bg-red-50 border border-red-200 p-3 rounded-lg shadow-sm w-72 text-center relative">
      <div className="text-sm font-bold text-red-900 mb-1">{data.title as string}</div>
      <div className="text-xs text-red-700">{data.story as string}</div>
      <Handle type="source" position={Position.Bottom} className="w-3 h-3 bg-white border-2 border-red-400" />
    </div>
  )
}
